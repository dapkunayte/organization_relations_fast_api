...
def create_organization(db: Session, organization: pydantic_models.Organization):
    ...
    db_organization_inn = db.query(models.Device).filter(models.Organization.uuid == organization.inn).first()
    db_organization_name = db.query(models.Device).filter(models.Organization.organization_name == organization.organization_name).first()
...
def get_devices(db: Session, skip: int = 0, limit: int = 100):
    ...
    if db_devices is None or db_devices == [ ]:
